<template>
  ...
  <div class="mx-auto mt-10">
    <div v-for="banner in bannerData" :key="banner.id" class="m-2">
      ...
      <div class="justify-between mt-2">
        ...
        <input
          ...
          id="title" type="text" v-model="banner.title" placeholder="Título">
        <!-- <button @click="editBanner(banner.id)"
          class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Salvar</button> -->
      </div>
      <div class="mb-4 w-full">
        <label class="flex text-gray-700 text-sm font-bold mb-2" for="status">Status:</label>
        <select
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          id="status" v-model="banner.status">
